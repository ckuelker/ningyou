...
sub apply {
    ...
    $s->p("         merge attributes of duplicated sections. Make a backup\n");
        ...
        if ( $fn =~ m{^$wt}gmx ) {    # is file in worktree?
            ...
            if ( -e $cfn ) {
                ...
                foreach my $sec ( $cfg->Sections ) {
                    ...
                    if (    $cfg->exists( $sec, "source" )
                        and $cfg->val( $sec, "source" ) eq $fn )
                    {
                        $s->d("OK\n");
                        $found = 1;
                        push @sec, $sec;
                        if ( $cfg->exists( $sec, "checksum" ) ) {
                        ...
                        #print "cs [$cs]\n";
                            ...
                            $s->e(
                                "section [$sec] in $cfn has not checksum attribute",
                                'cfg'
                            );
                        }
                    }
